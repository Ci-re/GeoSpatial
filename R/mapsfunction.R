...
# saveRDS(bio, "2bio.rds")
# prec_data <- readRDS("2bio.rds")
# prec_mean_nigeria <- raster::mask(prec_data$bio12, as_Spatial(lev1))
# prec_mean <- as.data.frame(prec_mean_nigeria, xy = TRUE, na.rm = TRUE)
# saveRDS(prec_mean,"prec_df.rds")
# prec_mean_nigeria_df <- readRDS("../")
...
render_maps <- function(dataframe, checks, trait, accessions, weather, switch){
  ...
  # print(weather_rasterFile)
  #
  # maps_data <- attach_locs(difference_dataframe)
  # print(maps_data)
